...
const AppointmentsPathologist = (props) => {
  const initialState = {
  ...
  useEffect(() => {
    try {
      var result = Pathologist.find(item => item._id === doctor);
  ...
  return (
    <div>{hospitaldata ?
      (<section className="text-gray-600 body-font overflow-hidden">
        ...
        <div id="defaultModal" tabindex="-1" aria-hidden="true" class={`fixed bg-slate-200 top-0 left-0  right-0 z-50 ${openmodel ? 'flex' : 'hidden'} justify-center w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full`}>
...
export default AppointmentsPathologist